...
def Fit_LSS(dim_c,initial_params=None):
    ...
    all_model = LSS(dim_u=2,dim_c=dim_c,dim_hidden=10,dim_out=1,name='all')
    
    all_model.A_eig = np.random.uniform(0.995,1,(dim_c))
    ...
    pkl.dump(results,open('LSS_c'+str(dim_c)+'_results.pkl','wb'))

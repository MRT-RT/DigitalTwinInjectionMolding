...
res_sim = pkl.load(open('MLP_press_sim_h'+str(dim_hidden)+'.pkl','rb'))
best_params = res_sim['params_val'].loc[res_sim['loss_val'].idxmin()]

# res = pkl.load(open('MLP_h5_sim_Hess.pkl','rb'))
# res = pkl.load(open('MLP_inj_h20_onestep_pred.pkl','rb'))


charges = list(range(3,5))    # 275
...
press_model.Parameters = best_params
...
_,res_train = parallel_mode(press_model,data_press_train)
_,res_val = parallel_mode(press_model,data_press_val)


idx = 0
plt.figure()
plt.plot(data_press_val['data'][idx][['p_wkz_ist','T_wkz_ist']])
plt.plot(res_val[idx][['p_wkz_ist','T_wkz_ist']])
plt.title('Num. of hidden neurons: '+str(dim_hidden))
plt.legend(['p_wkz','T_wkz','p_wkz_est','T_wkz_est'])
